...
async def pubmed_search_async(search_queries, top_k_results=5, email=None, api_key=None, doc_content_chars_max=4000):
    ...
    async def process_single_query(query):
        try:
            logging.info(f"Processing PubMed query: '{query}'")
            ...
            logging.info(f"Query '{query}' returned {len(docs)} results")
            ...
            import traceback
            logging.info(traceback.format_exc())  # Print full traceback for debugging
...
async def pubmed_search(queries: List[str]) -> str:
    ...
    raw = await pubmed_search_async(
        queries,
        top_k_results=params.get("top_k_results", 1),
        email=params.get("email"),
        api_key=params.get("api_key"),
        doc_content_chars_max=params.get("doc_content_chars_max", 5000),
    )

    # Deduplicate and format results into a markdown string
...
def _create_custom_chat_model(model_name: str) -> ChatOpenAI:
    ...
    return ChatOpenAI(
        ...
        api_key=os.getenv("CUSTOM_API_KEY", "EMPTY"),  # can be dummy
        model_name=model_name,
    )
from dotenv import load_dotenv
load_dotenv()
# def init_chat_model(model: str, **kwargs):
#     """
#     A thin wrapper around `langchain.chat_models.init_chat_model`.
#     If the *model* begins with ``custom:``, we route the request to the
#     self-hosted endpoint, otherwise we fall back to LangChain's stock
#     initialiser.

#     Example
#     -------
#     >>> init_chat_model("custom:mixtral-8x7b-instruct")
#     >>> init_chat_model("groq:llama2-70b-4096")
#     """
#     # 1) Ollama prefix: ollama:<model_name>
#     if model.startswith("ollama:"):
#         print("Using ollama")
#         return ChatOllama(
#             base_url=os.getenv("OLLAMA_BASE_URL", "http://localhost:11434"),
#             model=model.split(":", 1)[1],     # e.g. "ollama:llama3:instruct"
#             temperature=0,
#         )

#     # 2) GROQ prefix: groq:<model_name>
...
#             model_name=model.split(":", 1)[1],  # e.g. "groq:llama2-70b-4096"
#             temperature=0,
#             **kwargs
#         )

#     # 3) Custom endpoint (eg. SGLang): custom:<model_or_path>
#     # if model.startswith("custom:"):
#     #     print("Using custom endpoint")
#     #     _, mval = model.split(":", 1)
#     #     return ChatOpenAI(
#     #         model_name=mval,
#     #         openai_api_base=os.getenv("CUSTOM_API_URL"),
#     #         openai_api_key="dummy",  # no key needed
#     #         temperature=0,

#     #     )

#      # 3) Custom endpoint (eg. SGLang): custom:<model_or_path>    if model.startswith("custom:"):
#         print("Using custom endpoint")
#         _, model_name = model.split(":", 1)
#         base_url = os.getenv("CUSTOM_BASE_URL")
#         api_key = os.getenv("CUSTOM_API_KEY", "sk-dummy-key")
        
#         if not base_url:
#             raise ValueError("CUSTOM_BASE_URL environment variable must be set")
            
#         print(f"Initializing custom chat model with base_url: {base_url}")
#     print("Using custom endpoint")
#     model_name = model.split(":", 1)[1]  
#     print("Using modeeeel", model_name)  
#     return ChatOpenAI(
#         model=model_name,
#         base_url=os.getenv("CUSTOM_BASE_URL"),
#         api_key=os.getenv("CUSTOM_API_KEY", "sk-dummy-key"),
...
#     from langchain.chat_models import init_chat_model as _orig_init
#     return _orig_init(model=model)
...
def _create_custom_chat_model(model_name: str, **common_kwargs) -> ChatOpenAI:
    ...
    return ChatOpenAI(
        base_url=base_url.rstrip("/"),
        api_key=os.getenv("CUSTOM_API_KEY", "sk-dummy-key"),  # ignored by many gateways
        model=model_name,
        **common_kwargs,
    )

# -----------------------------------------------------------------------------
# Public – polymorphic factory
# -----------------------------------------------------------------------------

def init_chat_model(model: str, *, temperature: float = 0, streaming: bool = True, **kwargs):
    """Return a chat model for *model*.

    • ``custom:<name>``   – self‑hosted OpenAI‑compatible gateway (FastChat, LM‑Studio, etc.)
    • ``groq:<name>``     – GroqCloud
    ...
    # ----------------------------  OpenAI  -------------------------
    # Fall back to the official OpenAI endpoint
